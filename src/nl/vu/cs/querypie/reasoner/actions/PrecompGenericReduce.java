...
public class PrecompGenericReduce extends Action {
  ...
  public void startProcess(ActionContext context) throws Exception {
    ...
    for (int m = 0; m < rules.length; ++m) {
      ...
      precompTuples[m] = incrementalFlag ? rule.getFlaggedPrecomputedTuples() : rule.getAllPrecomputedTuples();
      if (pos_gen_precomps[m].length > 1) {
        throw new Exception("Not supported");
      }

      // Fill the outputTriple with the constants that come from the head of the rule
